...
export class PlayerData {
  static async register(player, obj, Discordid = player.getSyncedMeta("Discordid")) {
    try {
      ...
      await sql_log(`INSERT INTO register_log(pId, loginas, ip, timestamp, hwid, discordid, license) VALUES ("${dataMain.insertId}","Server","${player.ip.substr(7, 24)}","${Date.now()}","${player.hwidHash}","${player.getSyncedMeta("Discordid")}","${player.socialID}")`);
    } catch (error) {
  ...
  static async login(player, obj) {
    try {
      ...
      if (DaTa.pPassword == undefined) return alt.emitClient(player, "CallBack_Login_Account_To_Server", false, "UsernameError");
      if ((await hashing.sjcl.verify(obj.password, DaTa.pPassword)) != true) return alt.emitClient(player, "CallBack_Login_Account_To_Server", false, "PasswordError");
      if (DaTa.pOnline == 1) return alt.emitClient(player, "CallBack_Login_Account_To_Server", false, "AccountOnline");
