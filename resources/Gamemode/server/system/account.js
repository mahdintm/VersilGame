...
export class PlayerData {
  static async register(
    ...
    try {
      let dataMain = await sql(
        `INSERT INTO Account(pName, pPassword, pPhonenumber, pEmail, pDiscord, pSex, pFirstName, pLastName, pNotificationEmail, pNotificationPhone, pBrithday, pLastLogin, pCreateAt) VALUES("${obj.main.Username.toLowerCase()}", "${await hashing.sjcl.ConvertToHash(
          obj.main.Password
        )}", "${obj.main.Phone}", "${obj.main.Email}", "${Discordid}", "${
          obj.main.RealSex
        }", "${
          obj.main.RealFirstName.charAt(0).toUpperCase() +
          obj.main.RealFirstName.slice(1)
        }", "${
          obj.main.RealLastName.charAt(0).toUpperCase() +
          obj.main.RealLastName.slice(1)
        }", "${obj.main.SendNotificationToEmail ? 1 : 0}", "${
          obj.main.SendNotificationToPhone ? 1 : 0
        }", "${obj.main.RealDateOfBirth}", "0", "${Date.now()}")`
      );
      await PlayerData.login(player, {
        username: obj.main.Username,
        password: obj.main.Password,
      });
      ...
      await sql_log(
        `INSERT INTO register_log(pId, loginas, ip, timestamp, hwid, discordid, license) VALUES ("${
          dataMain.insertId
        }","Server","${player.ip.substr(7, 24)}","${Date.now()}","${
          player.hwidHash
        }","${player.getSyncedMeta("Discordid")}","${player.socialID}")`
      );
    } catch (error) {
  ...
  static async login(player, obj) {
    try {
      let DaTa = await sql(
        `select * FROM Account WHERE pName = "${obj.username.toLowerCase()}"`
      );
      ...
      let time = Date.now();
      await sql(
        `update Account set pOnline="${1}",pLastLogin="${time}" WHERE pId="${
          DaTa.pId
        }"`
      );
      ...
      //log system
      await sql_log(
        `INSERT INTO login_log(pId, loginas, ip, timestamp, hwid, discordid, license) VALUES ("${
          DaTa.pId
        }","Server","${player.ip.substr(7, 24)}","${time}","${
          player.hwidHash
        }","${player.getSyncedMeta("Discordid")}","${player.socialID}")`
      );
      DiscordHook.newhook.login({
      ...
      return alt.log(
        `~lc~ Player ${obj.username} With IP ${player.ip.substr(
          7,
          24
        )} at ${time} Logined.`
      );
  ...
  static async set(player, DataName, value, sync_sql = false) {
    try {
      ...
      if (sync_sql)
        await sql(
          `UPDATE Account SET ${DataName} = '${value}' WHERE pId = "${await PlayerData.get(
            player,
            "pId"
          )}"`
        );
  ...
  static get = {
    ...
    async idGameFromPlayerObject(player) {
      try {
        ...
        await logger.addlog.server({
          locatin:
            "Server->System->Account->Class playerIdGame->idGameFromPlayerObject()",
          message: error,
        });
      }
    },
    ...
    async PlayerObjectFromGameid(enterdid) {
      try {
        ...
        await logger.addlog.server({
          locatin:
            "Server->System->Account->Class playerIdGame->PlayerObjectFromGameid()",
          message: error,
        });
      }
    },
...
export class FindPlayerAccount {
  static async Name(value) {
    let id = Object.entries(playersdata).filter(
      (v, index, ar) =>
        v[1].pName.toLowerCase().match(value.toLowerCase()) != null
    );
