...
export class logger {
    ...
    static addlog = {
        ...
        async chat(FromServer, ChatLocation, player, LogDetail) {
            ...
            if (!fs.existsSync(`./logger/${year_month}/chat/${FromServer}/${ChatLocation}.json`))
                await fs.promises.writeFile(`./logger/${year_month}/chat/${FromServer}/${ChatLocation}.json`, '{}', "utf-8");
            let log = JSON.parse(await fs.promises.readFile(`./logger/${year_month}/chat/${FromServer}/${ChatLocation}.json`, "utf-8"));
            if (log[sqlid] == undefined)
                log[sqlid] = []
        ...
        async server(msg) {
            ...
            let year_month = logger.get_year_month(date)
            await logger.createfolders(year_month)
            if (!fs.existsSync(`./logger/server_err.json`))
                await fs.promises.writeFile(`./logger/server_err.json`, '{}', "utf-8");
