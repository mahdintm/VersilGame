...
export class StaffSystem {
  ...
  static Warn = {
    ...
    Helpers: async (msg) => {
      ...
      for (let i = 0; i < allPlayers.length; i++) {
        ...
        sendchat(allPlayers[i], `{${await Colors.chat.HelperWarn}}` + (await Language.GetValue(allPlayers[i].getSyncedMeta("Language"), "CHAT_HELPER_WARN")) + `{${Colors.chat.Default}} ` + msg);
      }
    },
    Leaders: async (msg) => {
      let allPlayers = alt.Player.all;
      for (let i = 0; i < allPlayers.length; i++) {
        ...
        sendchat(allPlayers[i], `{${await Colors.chat.LeaderWarn}}` + (await Language.GetValue(allPlayers[i].getSyncedMeta("Language"), "CHAT_LEADER_WARN")) + `{${Colors.chat.Default}} ` + msg);
      }
    },
    Staff: async (msg) => {
      let allPlayers = alt.Player.all;
      for (let i = 0; i < allPlayers.length; i++) {
        ...
        sendchat(allPlayers[i], `{${await Colors.chat.StaffWarn}}` + (await Language.GetValue(allPlayers[i].getSyncedMeta("Language"), "CHAT_STAFF_WARN")) + `{${Colors.chat.Default}} ` + msg);
      }
    },
  };
    ...
    if (datas.length != undefined) {
      datas.forEach(async (data) => {
        if (data.pOnline == 1) {
          ...
          if (playerid) this.CalculatorSP(await alt.Player.getByID(playerid));
        } else {
  ...
  static async CalculatorSP(player) {
    ...
    else if (staffpoint.week[NowNameDay] >= 16200000) await StaffPoint.give(player, 2);
    else if (staffpoint.week[NowNameDay] >= 10800000) await StaffPoint.give(player, 1);
    else if (staffpoint.week[NowNameDay] < 5400000) await StaffPoint.take(player, 1);
  }
  static async #CalculatorSP_DB(data) {
    let staffpoint = JSON.parse(data.pStaff_Point);
    let NowNameDay = await Time.GetNameDay();
    if (staffpoint.week[NowNameDay] >= 36000000) staffpoint.staff_point += 4;
    else if (staffpoint.week[NowNameDay] >= 25200000) staffpoint.staff_point += 3;
    else if (staffpoint.week[NowNameDay] >= 16200000) staffpoint.staff_point += 2;
    else if (staffpoint.week[NowNameDay] >= 10800000) staffpoint.staff_point += 1;
    else if (staffpoint.week[NowNameDay] < 5400000) staffpoint.staff_point -= 1;
  ...
  static async CalculatorRoleAll() {
    ...
    if (datas.length != undefined) {
      datas.forEach(async (data) => {
        if (data.pOnline == 1) {
          let playerid = await FindPlayerAccount.FromReferral(data.pId);
          if (playerid) this.CalculatorRole(await alt.Player.getByID(playerid));
        } else {
  ...
  static async #CalculatorRole_DB(data) {
    ...
    if (Sp < (await ServerSetting.get("lowestSpHelper_1"))) {
      ...
      if (data.pAdmin >= 1 && data.pCanAdmin) {
        ...
        if (data.pCanAdmin) return await sql(`update Account set pAdmin="2",pLeader="0",pHelper="3" where pId="${data.pId}"`);
        else return await sql(`update Account set pAdmin="0",pLeader="0",pHelper="3" where pId="${data.pId}"`);
      }
      ...
      if (data.pAdmin >= 1 && data.pCanAdmin) {
        ...
        if (data.pCanAdmin) return await sql(`update Account set pAdmin="3",pLeader="0",pHelper="3" where pId="${data.pId}"`);
        else return await sql(`update Account set pAdmin="0",pLeader="0",pHelper="3" where pId="${data.pId}"`);
      }
